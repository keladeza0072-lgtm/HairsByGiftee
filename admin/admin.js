...
document.querySelector("#loginForm").onsubmit=async e=>{e.preventDefault();const f=new FormData(e.currentTarget),err=document.querySelector("#loginError");err.hidden=true;try{await signInWithEmailAndPassword(auth,f.get("email"),f.get("password"))}catch(ex){err.textContent="That email or password was not accepted.";err.hidden=false}};
